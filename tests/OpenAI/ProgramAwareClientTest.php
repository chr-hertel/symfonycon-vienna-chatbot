    ...
    public function testProgramGetsPrepended(): void
    {
        $innerClient = new MockClient(
            assertion: function (array $messages) {
                ...
                self::assertStringContainsString('A Practical Introduction to Symfony UX', $systemPrompt, 'Expected program to be part of the system prompt.');
                self::assertStringContainsString('Vienna', $systemPrompt, 'Expected Vienna named as location in system prompt.');
                self::assertSame('user', $messages[1]['role'], 'Expected user message to be second message');
                self::assertSame('Can I learn something about Symfony UX?', $messages[1]['content'], 'Expected original user message as content.');
            },
            response: 'Yes, you can learn about Symfony UX. There is a workshop about it.',
        );
        $client = new ProgramAwareClient($innerClient, dirname(__DIR__, 2).'/symfonycon-program.txt');

        $messages = [['role' => 'user', 'content' => 'Can I learn something about Symfony UX?']];
        $response = $client->generateResponse($messages);

        self::assertSame('Yes, you can learn about Symfony UX. There is a workshop about it.', $response);
    }
